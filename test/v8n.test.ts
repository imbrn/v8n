...
import v8n from '../src/v8n';
import ValidationError from '../src/ValidationError';
import { AsyncRuleValidator, V8nValidator } from '../types/v8n';
  ...
  describe("the 'testAsync' function", () => {
    ...
    describe('the returned Promise', () => {
      ...
      it('should get correct ValidationException from composite failure', async () => {
        function asyncCompositeRule(): AsyncRuleValidator {
          return value =>
            new Promise(resolve => {
              v8n()
                .schema({
...
describe('rules', () => {
  ...
  describe('schema', () => {
    let is: V8nValidator;
    let not: V8nValidator;
    let validObj: Record<string, unknown>;
    let invalidObj: Record<string, unknown>;
...
describe('validation composition', () => {
  ...
  beforeEach(() => {
    ...
    const mockRule = new Rule('equal', () => true);
    causes = [
      new ValidationError(
        mockRule,
        10,
        [new ValidationError(mockRule, 10, [], 'two_one')],
        'two',
      ),
      new ValidationError(
        mockRule,
        10,
        [new ValidationError(mockRule, 10, [], 'three_two')],
        'three',
      ),
    ];
  });
...
function debugRules(validation: V8nValidator) {
